...
async def create_todos(task: TodoBase, db: Session = Depends(get_db)):
    ...
    return db_todos  # Повертаємо створений об'єкт

@app.patch('/tasks/{task_id}')
async def edit_task(task_id: int, task: str, rate: int, db: Session = Depends(get_db)):
    ...
    return db_todos  # Повертаємо оновлений об'єкт
...
async def delete_task(task_id: int, db: Session = Depends(get_db)):
    ...
    return {"message": "Task deleted successfully"}  # Повертаємо підтвердження видалення
